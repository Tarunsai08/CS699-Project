...
users_col = db['users']       
hospitalsCol = db['hospitals']
donors_col = db["donors"]  
organRequestsCol = db['organ_requests']
...
def signup(): 
    if request.method == 'POST':
        ...
        if not bloodGroup or bloodGroup not in valid_bloodGroups:
...
def login():
    if request.method == "POST":
        ...
        #check if the data given or not  
        if not username:
...
def admin_dashboard():
    ...
    if query:
        hospitals = list(hospitalsCol.find({
            "$or": [
...
def admin_new_hospital():
    if request.method == 'POST':
        ...
        if hospitalsCol.find_one({'username': username}):
            flash("Hospital username already exists", "danger")
            return redirect(url_for('admin_new_hospital'))

        hospitalsCol.insert_one({
            "username": username,
...
def admin_edit_hospital(hos_id):
    hospital = hospitalsCol.find_one({'_id': ObjectId(hos_id)})
    if not hospital:
    ...
    if request.method == "POST":
        ...
        existing = hospitalsCol.find_one({'username': username, '_id': {'$ne': hospital["_id"]}})
        if existing:
            flash("username already in use.", 'danger')
            return redirect(request.url)
  
        if not display_name or not username:
            flash("Username and Institution Name are to be mentioned definelty", 'danger')
            return redirect(request.url)
        ...
        if password:  
            updateData['password_hash'] = generate_password_hash(password)

        hospitalsCol.update_one(
            {'_id': ObjectId(hos_id)},
            {"$set": updateData}
        )

        flash('Hospital is updated', "success")
        return redirect(url_for('admin_dashboard'))
...
def admin_delete_hospital(hos_id):
    ...
    hospitalsCol.delete_one({'_id': ObjectId(hos_id)})

    flash('Hospital and all related donors are deleted from the DB .', "info")
    return redirect(url_for("admin_dashboard"))
...
def admin_handle_request_action(req_id):
    ...
    req = organRequestsCol.find_one({'_id': ObjectId(req_id)})

    if not req:
        flash("Request is not found", "danger")
        return redirect(url_for('admin_requests'))

    if action == "reject":
        organRequestsCol.update_one(
            {"_id": req['_id']},
    ...
    if action == "approve":
        ...
        best_donor, bestscore = allocate_for_request(req)

        if best_donor:
            flash("organ successfully allocated to user", "success")
        else:
            flash("No suitable donor found, request is failed", "warning")
...
@login_required(role="admin")
def admin_requests():
    reqs = list(organRequestsCol.find().sort('created_at', -1))
...
def hospital_login():
    if request.method == "POST":
        ...
        hospital = hospitalsCol.find_one({'username': username})
        if not hospital:
...
def hospital_edit_donor(donor_id):
    ...
    # make it sure that this donor belongs to this hospital only
...
def score_match(doc,user_age, user_blood,valid_groups_donars) :
    s = 0

    try:
        docbg = str(doc.get("Blood_Type", '')).strip()
        if docbg and docbg.lower() == user_blood.lower():
            s += 150

        elif docbg in valid_groups_donars:
            s += 100
        else:
    ...
    try:
        docage = int(doc.get('Age') )
        age_diff = abs( docage-user_age)
...
def search():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(url_for('index'))
        user_blood = user.get('bloodGroup', '').strip()
        if not user_blood:
        ...
        valid_groups_donars = blood_compatibility.get(user_blood, [])

        organ_ages_allowed_to_transplant = {
            "Kidney": 15,
            "Liver": 20,
            'Heart': 10,
            "Lung": 10,
        ...
        tol = organ_ages_allowed_to_transplant.get(organ.title(), organ_ages_allowed_to_transplant['Default'])
        ...
        if valid_groups_donars:
            query["Blood_Type"] = {'$in': valid_groups_donars}
        ...
        for c in candidates:
            c['_score'] = score_match(c, user_age, user_blood, valid_groups_donars)
        ...
        hospital_map = {
            str(h['_id']):h
            for h in hospitalsCol.find({'_id'  :{"$in":hospital_ids}})
        }
        ...
        if upload_days:
            ...
            plt.savefig(fpath, bbox_inches='tight')
            plt.close()
            charts.append({"label":'Matchestimeline','filename':fname})
...
def allocate_for_request(request_doc):
    ...
    query = {
        ...
        'Blood_Type': {'$in': valid_groups_donars},
    ...
    for d in candidates:
        s = score_match(d, user_age, user_blood, valid_groups_donars)
        if s > best_score:
            best_score = s
            best_donor = d
    ...
    #mark donar as allocated and should not be shown in the any other requests
    donors_col.update_one(
        {"_id": best_donor["_id"]},
        {'$set': {
            'is_allocated': True,
    ...
    #mark the requests as matched already so that Ui will fetch it and display it to user
    organRequestsCol.update_one(
        {"_id": request_doc['_id']},
        {"$set": {
            'status': "matched",
...
def request_organ():
    if request.method == 'POST':
        ...
        organRequestsCol.insert_one(req)
...
def my_requests():
    user = users_col.find_one({'username': session["user"]})
    reqs = list( organRequestsCol.find({'user_id': user["_id"]}) )
    donor_ids = [  
    ...
    for req in  reqs:
        ...
        if donor_id:
            ...
            if donor:
                ...
                req['donor_name'] ="anonymous Donor"
                req['hospital_name'] ="anonymous Hospital"          
